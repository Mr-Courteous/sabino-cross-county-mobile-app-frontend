...
export default function StudentVerifyEmail() {
    ...
    return (
        <ThemedView style={{ flex: 1, backgroundColor: Colors.accent.navy }}>
            <ImageBackground source={{ uri: 'https://images.unsplash.com/photo-1523240795612-9a054b0db644?q=80&w=2070' }} style={styles.hero}>
                <LinearGradient colors={['rgba(10, 15, 30, 0.8)', 'rgba(15, 23, 42, 0.98)']} style={styles.overlay}>
                    <ScrollView contentContainerStyle={styles.scrollContainer} showsVerticalScrollIndicator={false}>
                        ...
                        <View style={styles.card}>
                            {!isTiny && <View style={styles.iconCircle}><Ionicons name="id-card" size={32} color="#FACC15" /></View>}
                            {error ? <CustomAlert type="error" title="Error" message={error} onClose={() => setError('')} style={{ marginBottom: 16 }} /> : null}
                            <CustomInput label="Student Email" placeholder="student@oags.com" value={email} onChangeText={setEmail} autoCapitalize="none" keyboardType="email-address" editable={!loading} />
                            <Text style={styles.infoText}>A secure handshake code will be sent to your inbox.</Text>
                            <CustomButton title={loading ? "..." : "GET OTP"} onPress={handleSendOtp} loading={loading} variant="premium" style={styles.ctaButton} />
                            <TouchableOpacity style={styles.loginLink} onPress={() => router.push('/(student)')}>
                                <Text style={styles.loginLinkText}>EXISTING STUDENT? <Text style={{ color: '#FACC15', fontWeight: '900' }}>SIGN IN</Text></Text>
                            </TouchableOpacity>
                        </View>
...
function makeStyles(width: number) {
    ...
    return StyleSheet.create({
        ...
        loginLinkText: { color: '#94A3B8', fontSize: 10, letterSpacing: 1, fontWeight: '700' },
