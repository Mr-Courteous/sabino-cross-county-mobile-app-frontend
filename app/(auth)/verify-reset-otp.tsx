                ...
                <LinearGradient colors={['rgba(10, 15, 30, 0.8)', 'rgba(15, 23, 42, 0.98)']} style={styles.overlay}>
                    <ScrollView contentContainerStyle={styles.scrollContainer} showsVerticalScrollIndicator={false}>
                        ...
                        <View style={styles.card}>
                            {!isTiny && <View style={styles.iconCircle}><Ionicons name="shield-checkmark" size={28} color="#FACC15" /></View>}
                            ...
                            <CustomInput label="6-Digit Reset Code" placeholder="000000" value={otp} onChangeText={(t) => setOtp(t.replace(/\D/g, '').slice(0, 6))} keyboardType="numeric" maxLength={6} editable={!verifying} inputStyle={styles.otpInput} />
                            <Text style={[styles.timerText, timeLeft <= 60 && { color: '#EF4444' }]}>{timeLeft > 0 ? `Code expires in ${formatTime(timeLeft)}` : 'Code expired'}</Text>
                            <CustomButton title={verifying ? "VERIFYING..." : "CONFIRM RESET"} onPress={handleVerifyOTP} disabled={verifying || timeLeft === 0} loading={verifying} variant="premium" style={styles.ctaButton} />
